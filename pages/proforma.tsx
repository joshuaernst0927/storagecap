...
export default function Proforma() {
  ...
  function handleGenerateLOI() {
    ...
    const gpFeeTotal = Math.round(offerPriceVal * (n(inputs.acquisitionFeePct, 2) / 100))
    const property_description = [
      inputs.totalUnits ? `${inputs.totalUnits}-unit` : '',
      'self-storage facility',
      inputs.totalSF ? `totaling ${inputs.totalSF} SF` : '',
      inputs.yearBuilt ? `built ${inputs.yearBuilt}` : '',
    ].filter(Boolean).join(' ')

    const underwriting_narrative = irrResult ? [
      `At the proposed offer price of $${offerPriceVal > 0 ? Math.round(offerPriceVal).toLocaleString() : inputs.offerPrice}, the asset underwrites to a ${irrResult.going_in_cap != null ? (irrResult.going_in_cap * 100).toFixed(2) + '% going-in cap rate' : 'going-in cap rate pending NOI confirmation'}.`,
      irrResult.levered_irr != null ? `Levered IRR is projected at ${(irrResult.levered_irr * 100).toFixed(1)}% with a ${waterfallResult ? waterfallResult.lpMOIC.toFixed(2) + 'x LP equity multiple' : 'LP equity multiple pending waterfall'}.` : '',
      inputs.currentOccupancy ? `Current occupancy is ${inputs.currentOccupancy}% with a ${inputs.monthsToStabilization || '24'}-month stabilization timeline underwritten.` : '',
    ].filter(Boolean).join(' ') : ''
    ...
    const data = {
      ...
      current_avg_rent: inputs.currentAvgRent,
      market_avg_rent: inputs.marketAvgRent,
      months_to_stabilization: inputs.monthsToStabilization,
      broker1_name: inputs.broker1Name,
      broker2_name: inputs.broker2Name,
      broker1_phone: inputs.brokerPhone1,
      broker2_phone: inputs.brokerPhone2,
      broker1_email: inputs.brokerEmail1,
      broker2_email: inputs.brokerEmail2,
      brokerage: inputs.brokerageName,
      salutation,
      offer_price: offerPriceVal > 0 ? String(Math.round(offerPriceVal)) : inputs.offerPrice,
      all_in_cost: String(Math.round(offerPriceVal + lpEquityTotal)),
      bridge_loan: String(bridgeLoan),
      bridge_rate: inputs.bridgeRate,
      annual_ds: String(Math.round(annualInterest)),
      interest_reserve: String(Math.round(annualInterest)),
      capex_reserve: inputs.capexReserve,
      gp_fee_total: String(gpFeeTotal),
      gp_fee_income: inputs.acquisitionFeePct,
      gp_coinvest: '0',
      lp_equity: String(Math.round(lpEquityTotal)),
      waterfall,
      going_in_cap: irrResult ? (irrResult.going_in_cap * 100).toFixed(2) : '',
      yr3_cap: y3Cap,
      pf_cap: pfCap,
      exit_cap: inputs.exitCapRate,
      lp_moic: waterfallResult ? waterfallResult.lpMOIC.toFixed(2) : '',
      lp_irr: irrResult ? ((irrResult.levered_irr ?? irrResult.unlevered_irr ?? 0) * 100).toFixed(1) : '',
      gp_moic: waterfallResult ? waterfallResult.gpMOIC.toFixed(2) : '',
      gp_irr: irrResult ? ((irrResult.levered_irr ?? irrResult.unlevered_irr ?? 0) * 100).toFixed(1) : '',
      emd: String(Math.round(offerPriceVal * 0.01)),
      dd_days: '30-45',
      closing_days: '30-45',
      t12_noi: inputs.t12NOI,
      year1_noi: String(Math.round(ourYears[0]?.noi ?? 0)),
      year2_noi: String(Math.round(ourYears[1]?.noi ?? 0)),
      year3_noi: String(Math.round(y3NOI)),
      year5_noi: String(Math.round(y5NOI)),
